...
本文记录 GEO Console 对 AI 联网回答进行重复采样、语义解析、确定性统计和趋势判断的下一版设计。它用于后续评审与实施，不代表仓库当前已经具备语义解析、指标 V2 或消费端 App 自动采集能力。
...
- 不在本设计阶段新增 migration、Worker、API、页面或运行配置。
...
该改造属于指标、证据派生、队列和报告工作流的重大功能变更。禁止一次性原地替换。建议按以下阶段实施：
...
在没有完成上述实现和验证前，当前产品继续使用 V1 指标和 `llm_search_api` 口径；本文不得作为已上线能力说明。
