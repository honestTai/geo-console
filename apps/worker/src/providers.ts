...
export async function ensureProviderConfigs(database: Database): Promise<void> {
	for (const id of searchProviderIds) {
		...
		await database.query(
			`INSERT INTO provider_configs
			 ...
			 VALUES ($1,'default',$2,false,$3,$4,$5,$6::jsonb,$7)
...
export async function saveProviderConfig(
	...
	if (data.apiKey) await writeEncryptedCredential(database, definition.credentialKey, data.apiKey);
	if (data.secondaryApiKey && definition.secondaryCredentialKey)
		await writeEncryptedCredential(database, definition.secondaryCredentialKey, data.secondaryApiKey);
	if (data.enabled) {
		const primary = await readEncryptedCredential(database, definition.credentialKey);
		const secondary = definition.secondaryCredentialKey
			? await readEncryptedCredential(database, definition.secondaryCredentialKey)
			: "not-required";
		if (!primary || !secondary) throw new Error("启用平台前必须配置所需 API 密钥");
	}
	await database.query(
		`UPDATE provider_configs SET enabled=$2,model=$3,endpoint=$4,
		 search_strategy=$5::jsonb,adapter_version=$6,updated_at=now() WHERE organization_id='default' AND provider_id=$1`,
		[
			providerId,
			data.enabled,
			data.model,
...
export async function getProviderSettings(database: Database): Promise<unknown[]> {
	const rows = await database.query<Record<string, unknown>>(
		"SELECT * FROM provider_configs WHERE organization_id='default' ORDER BY provider_id",
	);
	return Promise.all(
		rows.rows.map(async (row) => {
			...
			return {
				...
				configured: Boolean(await readEncryptedCredential(database, definition.credentialKey)),
				secondaryConfigured: definition.secondaryCredentialKey
					? Boolean(await readEncryptedCredential(database, definition.secondaryCredentialKey))
					: null,
				model: row.model,
...
export async function buildProviderAdapter(
	...
	const row = (
		await database.query<Record<string, unknown>>(
			"SELECT * FROM provider_configs WHERE organization_id='default' AND provider_id=$1",
			[providerId],
		)
	).rows[0];
	if (!row) throw new Error(`平台 ${providerId} 尚未初始化`);
	const definition = providerDefinitions[providerId];
	const apiKey = await readEncryptedCredential(database, definition.credentialKey);
...
export async function testProviderConfig(database: Database, providerId: SearchProviderId): Promise<unknown> {
	const result = await (await buildProviderAdapter(database, providerId)).testConnection();
	await database.query(
		`UPDATE provider_configs SET last_test_status=$2,last_test_message=$3,last_tested_at=now(),updated_at=now()
		 WHERE organization_id='default' AND provider_id=$1`,
		[providerId, result.status, result.message],
	);
	return result;
}
