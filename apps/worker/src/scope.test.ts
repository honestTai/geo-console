...
describe("监测范围版本", () => {
	it("保留旧批次范围并允许同一竞品在新版本继续使用", async () => {
		...
		try {
			...
			const savedBatch = (
				await database.query<{ config: { prompts: Array<{ id: string; question: string }> } }>(
					"SELECT config FROM experiment_batches WHERE id=$1",
					[batch.id],
				)
			).rows[0];
			expect(savedBatch.config.prompts).toEqual([
				{ id: "prompt-v1", question: "旧问题是什么？", intent: "购买", tags: [] },
			]);
