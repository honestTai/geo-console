...
export function websiteAuditSection(result: WebsiteAuditResult, screenshot?: string): string {
	...
	<h3>执行摘要与处理顺序</h3><p>${checks.filter((c) => c.status === "fail").length} 项阻断、${checks.filter((c) => c.status === "warning").length} 项需优化。先处理公开访问与 HTTPS，再处理发现路径、页面结构与主体事实；参考项不作为收录承诺。</p>
	${checks
		.map((check) => {
			const guide = websiteGuidance[check.id];
			return `<article class="zz-audit-check"><h3>${e(check.id)} · ${e(check.label)} · ${states[check.status]}</h3><p><b>观察事实：</b>${e(check.detail)}</p><p><b>定位：</b>${e(check.selector ?? guide?.selector ?? "见源证据")}</p><p><b>处理建议：</b>${e(check.recommendation ?? guide?.recommendation ?? "复核源证据")}</p><p><b>验收办法：</b>${e(check.verification ?? guide?.verification ?? "在相同 URL 重新审计")}</p><p><b>证据引用：</b>${e(check.evidenceIds?.join("、") || "历史审计未提供逐项源文件")}</p></article>`;
		})
		.join("")}
	...
	<h3>原始证据索引</h3>${(result.evidence ?? []).map((item) => `<article><b>${e(item.id)} · ${e(item.kind)}</b><p>${e(item.url)} · HTTP ${e(item.status ?? "不适用")} · SHA-256 ${e(item.contentHash ?? "无原始正文")}</p>${item.error ? `<p>${e(item.error)}</p>` : ""}${item.excerpt ? `<pre>${e(item.excerpt)}</pre>` : ""}</article>`).join("")}
