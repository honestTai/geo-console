...
export async function sweepTerminalLeases(database: Database): Promise<number> {
	const expired = await database.query<{
		...
		WHERE type IN ('agent_draft','agent_session_turn','report_document','report_pdf','semantic_parse','answer_analysis','article_quality') AND status='leased'
		AND lease_expires_at<now() AND attempts>=max_attempts RETURNING type,payload`);
	for (const row of expired.rows) {
		if (row.type === "article_quality")
			await database.query(
				"UPDATE article_quality_runs SET status='failed',error_message='执行租约已过期且重试耗尽',completed_at=now(),updated_at=now() WHERE id=$1 AND status IN ('pending','running')",
				[row.payload.qualityId],
			);
		if (row.type === "answer_analysis")
			await database.query(
				"UPDATE answer_analysis_runs SET status='failed',error_message='执行租约已过期且重试耗尽',completed_at=now(),updated_at=now() WHERE id=$1 AND status IN ('queued','running')",
				[row.payload.analysisId],
			);
