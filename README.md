...
- AI 工作台：正式桌面客户端在本机 WebView 内运行 HRouter Agent 的推理与工具编排，每台客户端独立并发；服务端只代理受控 Responses 流、执行项目范围内的业务工具并保存会话/证据。输入“跑基线/跑快审/生成报告/生成优化文章”等自然语言指令后，Agent 确认问题集并调用服务端接口建批次、等待采集、核验、官网审计、规则诊断、报告叙述/质检/冻结/PDF 和优化文章；浏览器入口保留服务端 Agent 作为调试兼容。
